...
begin
  try
    ...
    TBDEV.SQL.Add('   JOIN IBI_MOTIVO_DEVOLUCOES M ON M.CD_MOTIVO = DV.CD_MOTIVO');
    TBDEV.SQL.Add('WHERE');
    ...
    TBDEVAR.SQL.Add('   JOIN IBI_MOTIVO_DEVOLUCOES M ON M.CD_MOTIVO = DV.CD_MOTIVO');
    ...
    TBDEVFAC.SQL.Add('   JOIN IBI_MOTIVO_DEVOLUCOES M ON M.CD_MOTIVO = DV.CD_MOTIVO');
    ...
    TBCONSOLIDA.SQL.Add('   JOIN IBI_MOTIVO_DEVOLUCOES M ON M.CD_MOTIVO = DV.CD_MOTIVO');
...
begin
  try
    ...
    TBDATAS.SQL.Add('   DISTINCT DT_DEVOLUCAO, ''DEV'' AS TPDEV');
    TBDATAS.SQL.Add('FROM');
    TBDATAS.SQL.Add('   CEA_CONTROLE_DEVOLUCOES');
    TBDATAS.SQL.Add('WHERE');
    TBDATAS.SQL.Add('       DT_DEVOLUCAO >= :XPDTINI1');
    TBDATAS.SQL.Add('   AND DT_DEVOLUCAO <= :XPDTFIM1');
    TBDATAS.SQL.Add('   AND ID >= :XPIDINI1');
    TBDATAS.SQL.Add('   AND ID <= :XPIDFIM1');
    TBDATAS.SQL.Add('   AND CODBIN '+MontarFiltroGrupoBIN(grupoId, familia));

    TBDATAS.SQL.Add('UNION');
    TBDATAS.SQL.Add('SELECT');
    TBDATAS.SQL.Add('   DISTINCT DT_DEVOLUCAO, ''DEV_AR'' AS TPDEV');
    TBDATAS.SQL.Add('FROM');
    TBDATAS.SQL.Add('   IBI_CONTROLE_DEVOLUCOES_AR');
    TBDATAS.SQL.Add('WHERE');
    TBDATAS.SQL.Add('       DT_DEVOLUCAO >= :XPDTINI2');
    TBDATAS.SQL.Add('   AND DT_DEVOLUCAO <= :XPDTFIM2');
    TBDATAS.SQL.Add('   AND ID >= :XPIDINI2');
    TBDATAS.SQL.Add('   AND ID <= :XPIDFIM2');
    TBDATAS.SQL.Add('   AND CODBIN '+MontarFiltroGrupoBIN(grupoId, familia));

    TBDATAS.SQL.Add('UNION');
    TBDATAS.SQL.Add('SELECT');
    TBDATAS.SQL.Add('   DISTINCT DT_DEVOLUCAO, ''DEV_FAC'' AS TPDEV');
    TBDATAS.SQL.Add('FROM');
    TBDATAS.SQL.Add('   IBI_CONTROLE_DEVOLUCOES_FAC');
    TBDATAS.SQL.Add('WHERE');
    TBDATAS.SQL.Add('       DT_DEVOLUCAO >= :XPDTINI3');
    TBDATAS.SQL.Add('   AND DT_DEVOLUCAO <= :XPDTFIM3');
    TBDATAS.SQL.Add('   AND ID >= :XPIDINI3');
    TBDATAS.SQL.Add('   AND ID <= :XPIDFIM3');
    TBDATAS.SQL.Add('   AND CODBIN '+MontarFiltroGrupoBIN(grupoId, familia));
...
begin
  try
    ...
    TBLIMITE.SQL.Add('   MIN(ID) AS MINIMO, MAX(ID) AS MAXIMO, ''DEV'' AS TPDEV');
    TBLIMITE.SQL.Add('FROM');
    TBLIMITE.SQL.Add('   CEA_CONTROLE_DEVOLUCOES');
    TBLIMITE.SQL.Add('WHERE');
    TBLIMITE.SQL.Add('   DT_DEVOLUCAO >= :XPDTINI1 AND DT_DEVOLUCAO <= :XPDTFIM1');
    TBLIMITE.SQL.Add('   AND CODBIN '+MontarFiltroGrupoBIN(grupoId, familia));
    TBLIMITE.SQL.Add('GROUP BY');
    TBLIMITE.SQL.Add('   3');
    TBLIMITE.SQL.Add('UNION');
    TBLIMITE.SQL.Add('SELECT');
    TBLIMITE.SQL.Add('   MIN(ID) AS MINIMO, MAX(ID) AS MAXIMO, ''DEV_FAC'' AS TPDEV');
    TBLIMITE.SQL.Add('FROM');
    TBLIMITE.SQL.Add('   IBI_CONTROLE_DEVOLUCOES_FAC');
    TBLIMITE.SQL.Add('WHERE');
    TBLIMITE.SQL.Add('   DT_DEVOLUCAO >= :XPDTINI2 AND DT_DEVOLUCAO <= :XPDTFIM2');
    TBLIMITE.SQL.Add('   AND CODBIN '+MontarFiltroGrupoBIN(grupoId, familia));
    TBLIMITE.SQL.Add('GROUP BY');
    TBLIMITE.SQL.Add('   3');
    TBLIMITE.SQL.Add('UNION');
    TBLIMITE.SQL.Add('SELECT');
    TBLIMITE.SQL.Add('   MIN(ID) AS MINIMO, MAX(ID) AS MAXIMO, ''DEV_AR'' AS TPDEV');
    TBLIMITE.SQL.Add('FROM');
    TBLIMITE.SQL.Add('   IBI_CONTROLE_DEVOLUCOES_AR');
    TBLIMITE.SQL.Add('WHERE');
    TBLIMITE.SQL.Add('   DT_DEVOLUCAO >= :XPDTINI3 AND DT_DEVOLUCAO <= :XPDTFIM3');
    TBLIMITE.SQL.Add('   AND CODBIN '+MontarFiltroGrupoBIN(grupoId, familia));
    TBLIMITE.SQL.Add('GROUP BY');
    TBLIMITE.SQL.Add('   3');
    s := TBLIMITE.SQL.Text;
...
begin
  ...
  try
    ...
    begin
      ...
      if TBFAMILIA.FieldByName('FAMILIA').AsString = 'SEM FAMILIA' then
      begin
        if 1 = 4 then
         Exit;
      end;
...
begin
  try
    ...
    begin
      Application.MessageBox('Não foi possível gerar a planilha !', 'Aviso', MB_OK+MB_ICONWARNING);
      Exit;
    end;
    xGerou := True;
...
{
    Query.First;
    xFamiliaAtu := Query.FieldByName('FAMILIA').AsString;
    xMotivoAtu  := Query.FieldByName('DS_MOTIVO').AsString;
    while not Query.Eof do
    begin
      if Query.FieldByName('FAMILIA').AsString <> '' then
      begin
        if (xFamiliaAnt <> xFamiliaAtu) then
        begin
          xLinha := xLinha + 1;
          xPlanilha.Cells[xLinha, 1] := Query.FieldByName('FAMILIA').AsString;
      ...
      xFamiliaAnt := Query.FieldByName('FAMILIA').AsString;
